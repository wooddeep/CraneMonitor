...
public class AreaSetting extends AppCompatActivity {
        ...
        View.OnClickListener onClickListener = new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (view.getId() == R.id.add_logo) {
                    ...
                    List<Area> paras = confLoad(context);
                    if (paras.size() <= 1) {
                        Toast toast = Toast.makeText(AreaSetting.this, "不能全删除(can't remove all)!", Toast.LENGTH_SHORT);
    ...
    public static JSONArray getAreaConfig(AreaDao dao) {
        JSONArray out = new JSONArray();
        List<Area> paras = dao.selectAll();

        try {
            for (Area protect : paras) {
